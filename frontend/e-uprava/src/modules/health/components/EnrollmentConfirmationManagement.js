...
const EnrollmentConfirmationManagement = () => {
  ...
  useEffect(() => {
    if (selectedMedicalRecord === "") {
      ...
      const filtered = confirmations.filter(
        (confirmation) =>
          confirmation.medicalRecordId === parseInt(selectedMedicalRecord),
      );
  ...
  const fetchConfirmations = async () => {
    try {
      ...
      setError(null);
      const data =
        await enrollmentConfirmationService.getAllEnrollmentConfirmations();
  ...
  const handleSubmit = async (data) => {
    try {
      const basePreparedData = {
        ...
        issuedAt: data.issuedAt.includes("T")
          ? `${data.issuedAt}:00`
          : `${data.issuedAt}T00:00:00`,
        validUntil: data.validUntil.includes("T")
          ? `${data.validUntil}:00`
      ...
      if (editingItem) {
        ...
        await enrollmentConfirmationService.updateEnrollmentConfirmation(
          editingItem.id,
          preparedData,
        );
      } else {
        ...
        await enrollmentConfirmationService.createEnrollmentConfirmation(
          preparedData,
        );
      }
      ...
      else setError("Greška pri čuvanju potvrde");
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm("Da li ste sigurni da želite da obrišete ovu potvrdu?")
    ) {
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        <strong>Napomena:</strong> Potvrda za upis može se izdati isključivo za
        dete koje ima evidentiran najmanje jedan lekarski pregled i najmanje
        jednu primljenu vakcinu.
      </div>
